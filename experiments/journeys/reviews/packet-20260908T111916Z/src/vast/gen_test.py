...
def chat(messages, max_tokens=300, temperature=1.0):
    r = post("/v1/chat/completions", {"model": "qwen3.8-27b-gist", "messages": messages, "max_tokens": max_tokens, "temperature": temperature})
    m = r["choices"][0]["message"]
    return (m.get("reasoning_content") or m.get("reasoning") or ""), (m.get("content") or ""), r["usage"]
...
for p in prompts:
    for temp in (1.0, 1.5):
        ...
        ids = post("/tokenize", {"model": "qwen3.8-27b-gist", "prompt": reasoning + content})["tokens"]
