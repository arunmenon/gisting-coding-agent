...
def load_gist(segments_path):
    ...
    return {"tools": to_str(tools), "segments": [(s["text"], to_str(s)) for s in gisted]}
...
def gist_system_text(system_text):
    ...
    for anchor, gist_string in GIST["segments"]:
        start = system_text.index(anchor, cursor)
        out.append(system_text[cursor:start]); out.append(gist_string)
