...
.eqline-unused{display:flex;flex-wrap:wrap;align-items:center;gap:12px;font-family:var(--mono);font-size:clamp(1rem,1.8vw,1.3rem);margin:.6rem 0 1.4rem;color:var(--ink2)}
...
<div class="title-wrap">
  ...
  <div class="byline">PAI</div>
</div>''')

# ---- 2 BLUF ----
slide('''
<div class="eyebrow">Bottom line up front</div>
<h2>The answer in three lines</h2>
...
<h2>Five parts, designed for any harness and model pair</h2>
<div class="grid3" style="margin-top:1rem">
  ...
  <div class="card"><div class="k">3 &middot; Proxy</div><div class="v">Swaps the fixed span for the Steno tokens and adopts the same harness adapter. No change to the agent or the engine&rsquo;s code; the served model carries the new token rows.</div></div>
</div>
...
# ---- 4 SPAN + RECIPE ----
slide('''
<div class="eyebrow">The span, and why it is per pair</div>
<h2>What the agent re-sends, measured on one pair</h2>
<div class="compbar">
...
<div style="display:flex;gap:48px;flex-wrap:wrap;margin-top:1rem">
  <div><div class="bignum copper">17.5&ndash;21k</div><div class="unit">fixed tokens per call, across the<br>span configurations we measured</div></div>
...
<div class="grid4" style="margin-top:.6rem">
  ...
  <div class="card good"><div class="k">4 &middot; Checks</div><div class="v">Pass the every-call invariance check on a fresh capture, then token parity once the model adapter is in.</div></div>
</div>
<p class="lead" style="margin-top:1rem">Already earning its keep: on real Claude Code traffic, the every-call check caught <b>15 per-call differences</b> the old rules missed.</p>
...
# ---- 5 PROOF ----
slide('''
<div class="eyebrow">Results on the pair studied &middot; Claude Code &times; Qwen3.8</div>
...
<div class="figrow">
  ...
  <div>
    ...
    <p class="caption">Threshold: twice each arm&rsquo;s unloaded median latency, about 8.5 s full and 8.0 s Steno. Short, fixed-output replays on one GPU class, mean of three repeats; they did not measure task quality at load or isolate prefix-cache effects. On a larger H200, Steno was level at peak and ahead under heavy load (next slide).</p>
  </div>
</div>''' % curve())

# ---- 6b H200 ----
slide('''
<div class="eyebrow">Second GPU &middot; H200 NVL, 143 GB</div>
<h2>On a larger GPU, the gain shows up under heavy load</h2>
<div class="grid3" style="margin-top:1rem">
  <div class="card"><div class="k">At each arm&rsquo;s peak</div>
    <div style="display:flex;gap:26px;flex-wrap:wrap;margin-top:.4rem">
      <div><div class="bignum petrol">85.3</div><div class="unit">full prompt</div></div>
      <div><div class="bignum plum">83.3</div><div class="unit">Steno 8:1 &middot; level</div></div>
    </div></div>
  <div class="card plum" style="border-color:#5a3f6e"><div class="k">128 sessions &middot; cap lifted</div>
    <div style="display:flex;gap:26px;flex-wrap:wrap;margin-top:.4rem">
      <div><div class="bignum" style="color:#8a9aa0">47.0</div><div class="unit">full prompt</div></div>
      <div><div class="bignum plum">80.7</div><div class="unit">Steno 8:1 &middot; 1.7x</div></div>
    </div></div>
  <div class="card plum" style="border-color:#5a3f6e"><div class="k">256 sessions &middot; cap lifted</div>
    <div style="display:flex;gap:26px;flex-wrap:wrap;margin-top:.4rem">
      <div><div class="bignum" style="color:#8a9aa0">46.3</div><div class="unit">full prompt</div></div>
      <div><div class="bignum plum">56.7</div><div class="unit">Steno 8:1 &middot; 1.2x</div></div>
    </div></div>
</div>
<p class="caption" style="margin-top:.6rem">replay requests per minute</p>
<p class="lead" style="margin-top:1rem">At each arm&rsquo;s best point the two run <b>level</b>. Pushed to 128 and 256 concurrent sessions with our client&rsquo;s connection cap removed, the full prompt fell to about 47 req/min as the cache filled, while Steno served <b>1.2 to 1.7 times more</b>. Per rental dollar, the H100 with Steno and the H200 without it come out roughly even (appendix B).</p>
<p style="margin-top:1rem"><span class="tag warn">peak: two repeats, H100-tuned settings &middot; heavy load: one run per condition</span></p>''')
...
<div class="grid2" style="margin-top:1rem">
  <div class="card good"><div class="k">Established on this pair</div>
    <ul class="clean good" style="margin:0">
      <li>Easy suite: 12/12 at every ratio. Hard suite: corrected 8:1 matched the full prompt, 16/16.</li>
      <li>At 8:1, input per turn down from 24.3k to 9.4k tokens.</li>
      <li>Short H100 replays: <b>2x rate within each arm&rsquo;s latency threshold, +44% peak</b>.</li>
      <li>A spend-safe lab that ran the recipe end to end.</li>
    </ul>
  </div>
  <div class="card warn"><div class="k">Not yet</div>
    <ul class="clean warn" style="margin:0">
      <li>The experiment suite on a second harness and distilled-model pair.</li>
      <li>Quality on held-out work, and at production load.</li>
      <li>Savings per successful task, and why the gain happens. An external review withdrew our first explanation.</li>
...
<p class="big-verdict" style="margin-top:1.6rem">The lever is real on one pair. Next, test whether it carries to a distilled model on another harness.</p>
...
<p class="lead">External review found a 100-connection limit in our own load generator; that limit, not the GPU, produced the number our explanation rested on. A follow-up confirmed it and we withdrew the explanation. The peak difference, measured well below that limit, survives the correction; the other measurement limits still apply.</p>
<div class="layers">%s</div>
<p class="caption" style="max-width:90ch">The model has 16 full-attention and 48 linear-attention layers; their contribution to the observed throughput difference has not been isolated.</p>''' % layers)
...
# ---- APPENDIX D: THE LAB ----
slide('''
<div class="eyebrow">Appendix D &middot; the lab</div>
<h2>The lab that ran this study</h2>
<div class="pipe">
  ...
  <div class="pnode">Evaluate</div><div class="parrow">&rarr;</div>
  <div class="pnode">Destroy</div>
</div>
...
<p class="caption">Most experiments after the first ran through this controller rather than by hand. The loop reaches compute through a provider-neutral interface; vast.ai is the first backend.</p>''')
