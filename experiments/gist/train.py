...
def main():
    ...
    # held-out sessions, not held-out turns: every turn of a held-out session stays out of training
    sessions = sorted(set(e.get("session", "?") for e in examples)); random.shuffle(sessions)
    held = set(); eval_set = []
    for s in sessions:
        if len(eval_set) >= args.eval_n: break
        held.add(s); eval_set = [e for e in examples if e.get("session", "?") in held]
    train_set = [e for e in examples if e.get("session", "?") not in held] or examples
    random.shuffle(train_set)
    print("held-out sessions: %d (%d turns)" % (len(held), len(eval_set)))
