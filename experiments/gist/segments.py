...
def main():
    ...
    import hashlib
    tools_hash = hashlib.sha256(json.dumps(tools, sort_keys=False, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
    json.dump({"ratio": RATIO, "tools_ratio": TOOLS_RATIO, "tools_hash": tools_hash, "tool_names": [t_["name"] for t_ in tools], "original_vocab_len": original_len, "first_gist_id": first_gist_id, "gist_count": next_gist,
