...
<div class="grid4" style="margin-top:1.2rem">
  <div class="card"><div class="k">Where it fits</div><div class="v">A new pillar under <b>Lattice</b>, alongside the meta-harness, adaptive routing and model distillation tracks. Gisting cuts the <b>fixed prompt</b> a coding agent re-sends on every call.</div></div>
...
<p class="lead" style="margin-bottom:1.4rem">Gisting&rsquo;s lever on total cost: the model reads <b>fewer tokens on every agent call</b>, so the same GPU does more work.</p>
<div class="grid4">
  <div class="card"><div class="k">Meta-harness</div><div class="v">Adapter that routes different harnesses to underlying models.</div></div>
  <div class="card"><div class="k">Adaptive routing</div><div class="v">Research track.</div></div>
  <div class="card"><div class="k">Model distillation</div><div class="v">Research track.</div></div>
  <div class="card plum" style="border-color:#5a3f6e"><div class="k">Gisting &middot; new</div><div class="v">Shrinks the fixed prompt every agent call carries. <b>This deck.</b></div></div>
</div>''')
...
<p class="caption">Measured on Claude Code with the Qwen tokenizer. Any harness that exposes tools pays a version of this, but its size and makeup differ, so the span study is rerun for every harness and model pair.</p>''')
...
<div class="flow">
  <div class="step" style="border-color:var(--copper)"><div class="n" style="color:var(--copper)">00 &middot; per pair</div><h3>Study the span</h3><p>Measure what the harness re-sends on every call: what is fixed, what is per-session, and its share of each call.</p></div>
...
<div class="grid2" style="margin-top:.4rem">
  <div class="card good"><div class="k">Reused as is</div><div class="v">Training loop, proxy, evaluation harness, and the automated GPU lab that runs them.</div></div>
...
<p class="lead">A distilled student model still receives the same fixed preamble on every call. Running the gisting recipe on it attacks both costs at once: a <b>smaller model</b> reading a <b>shorter prompt</b>.</p>
...
<div class="grid3" style="margin-top:1rem">
  <div class="card plum" style="border-color:#5a3f6e"><div class="k">1 &middot; Next pair</div><div class="v">Run the experiment suite, span study first, on the next <b>harness and distilled-model pair</b>. Shows the recipe carries.</div></div>
  ...
  <div class="card"><div class="k">3 &middot; Guarded pilot</div><div class="v">Proposed for the Jetstream inner loop, with rule-audit and write-target checks at the serving boundary.</div></div>
</div>
<p class="big-verdict" style="margin-top:1.6rem">The lever is real on one pairing. Next, show it carries to a distilled model on another harness.</p>
...
<p class="caption">Every new harness and model pair will hit this boundary, which is why the span study comes first.</p>''')
